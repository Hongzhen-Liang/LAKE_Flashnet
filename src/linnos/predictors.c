...
#define CPU_Inference_FILE "/root/CPU_Inference_FILE_1"
int n_time=1;
...
bool cpu_prediction_model(char *feat_vec, int n_vecs, long **weights) {
	...
	bool end;
	for (int hongz=0; hongz<n_time; hongz++) {
	...
	s64 inference_latency = ktime_get_ns()-inference_start;
	pr_warn("CPU inference, n_time is %d, inference_latency is %lld\n",n_time,inference_latency);
	print_inference_time(CPU_Inference_FILE,inference_latency);
